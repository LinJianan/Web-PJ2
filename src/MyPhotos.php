<html>
    ...
    function Delete() {
        ...
        $sql = "DELETE FROM travelimage WHERE UID = $UID and ImageID = $delete";
        $check_query = mysqli_query($conn, $sql);
        $sql = "DELETE FROM travelimagefavor WHERE ImageID = $delete";
        $check_query = mysqli_query($conn, $sql);
        return;
    }
    ...
    function getImageID() {
        ...
        global $UID;
        $ImageID = array();
        $sql = "SELECT DISTINCT ImageID FROM travelimage WHERE UID = $UID";
        $check_query = mysqli_query($conn, $sql);
    ...
    <div class="content" style="position: relative;">
        ...
        elseif ($length == 0) {
            ...
            <font size=5>No Favor Now</font></p>';
        }
        else {
            while ($times < 3 && $remain > 0) {
                ...
                echo '<a href="Upload.php?ImageID='.$ImageID[3 * ($pagenum - 1) + $times].'&operation=1"><button style="
                position: absolute; left:550px; top: '.(250 + 300 * $times).
                'px; width:100px; height:30px;">Modify</button></a>';
                $times++;
                $remain--;
            }
        }
